...
do_compile:append() {
    ...
    # NOTE: passing -Dc_args/-Dcpp_args to `meson setup` overrides the OE
    # cross-file compiler args, which would drop the -f*-prefix-map flags and
    # bake the build path (recipe-sysroot/...) into the .so debug info ->
    # walnascar `buildpaths` QA is fatal. Re-inject ${DEBUG_PREFIX_MAP} so the
    # postprocess libs are mapped exactly like the meson.bbclass-built plugin.
    for subdir in ${S}/dx_stream/custom_library/postprocess_library/*/; do
        if [ -d "$subdir" ] && [ -f "$subdir/meson.build" ]; then
            ...
            meson setup build \
                ...
                --buildtype=release \
                --prefix=/usr \
                -Dcpp_args="${DEBUG_PREFIX_MAP} -I${S}/install/usr/include" \
                -Dc_args="${DEBUG_PREFIX_MAP} -I${S}/install/usr/include" \
                -Dcpp_link_args="${DEBUG_PREFIX_MAP} -L${S}/install/usr/lib/gstreamer-1.0 -L${S}/install/usr/lib" \
                -Dc_link_args="${DEBUG_PREFIX_MAP} -L${S}/install/usr/lib/gstreamer-1.0 -L${S}/install/usr/lib"
