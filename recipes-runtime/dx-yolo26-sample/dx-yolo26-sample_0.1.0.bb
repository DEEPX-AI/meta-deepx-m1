...
# 1. Source Definition
# The bundle is not fetched from a server - drop the tarball next to this recipe:
#   recipes-runtime/dx-yolo26-sample/files/dx-yolo26-sample_${PV}.tar.gz
#
# It is expected to contain a single top-level directory (hence
...
#
# NOTE: models plus video clips make for a large binary blob. If the bundle grows
# beyond what should live in a git layer, publish it and switch to a remote fetch:
...
# 2. Installation path
# The applications resolve both their default model ("assets/models/<file>", declared
# by the application's factory) and their default input image ("sample/img/<file>")
# relative to the working directory, so keeping the bundle layout intact makes
#   cd ${SAMPLE_DEST_DIR} && yolo26n_async
# work with no arguments at all.
SAMPLE_DEST_DIR = "${datadir}/dx-yolo26-sample"
...
# Uncomment if the bundle ships a helper shell script:
#RDEPENDS:${PN} += "bash"
